...
.done()).put(function(req, res, next){
	...
	return ((req.query.binary != null) ?
		...
		.spread((signedURL, getURL) => _write(getURL, {
            putURL: signedURL,
            getURL
        }
        ))
	...
	.catch(next)
	.done();}).delete(function(req, res, next){
	...
	.then(function(count){
		...
		return res
		.json(result)
		.end();}).catch(next);}).all((req, res, next) => next(new errors.InvalidMethodError()));
...
.all(function(req, res, next){
	...
	return next();}).get((req, res, next) => xtralife.api.virtualfs.read(req.context, req.params.domain, req.gamer._id, null)
.then(function(value){
