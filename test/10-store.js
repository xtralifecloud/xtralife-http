...
class InAppVerifierMock {
	constructor(originalIapObject) {
		this.originalIapObject = originalIapObject;
		this.appStoreMockConfig = { firstCode: 0, syntaxError: false };
		this.googleMockConfig = { validateSuccessfully: true };
		null;
	}

	// Call this before validating an appstore purchase
	configureNextAppleRequest(firstServerReturn, wantSyntaxError) {
		this.appStoreMockConfig.firstCode = firstServerReturn || 0;
		return this.appStoreMockConfig.syntaxError = wantSyntaxError || false;
	}

	// Call this before validating a google play purchase
	configureNextGoogleRequest(validateSuccessfully) {
		return this.googleMockConfig.validateSuccessfully = validateSuccessfully;
	}
	...
	verifyPayment(platform, payment, callback) {
		...
		if (platform === 'google') {
			...
			if (!this.appStoreMockConfig.syntaxError) {
				response =
					{ status: this.appStoreMockConfig.firstCode };
				if (this.appStoreMockConfig.firstCode === 0) {
					response.receipt = {
						in_app: [{
...
describe('Store', function () {
	...
	before('should wait for initialisation', function (done) {
		shuttlePromise.then(function (_shuttle) {
			shuttle = _shuttle;
	...
	before("should create an anonymous user", function (done) {
		...
		request(shuttle)
			...
			.end(function (err, res) {
				...
				return xtralife.api.store.TEST_clearStoreTransaction('googleplay.12999763169054705758.1368240874747405', function (err) {
					if (err != null) { return done(err); }
					return xtralife.api.store.TEST_setProductDefinitions(res.body.games[0].appid, definitions, err => done(err));
				});
			});
		return null;
	});
	...
	it("should not validate the same receipt twice", function (done) {
		inappMock.configureNextAppleRequest(0);
	...
	it("should not validate cryptographically incorrect receipt for Google product", function (done) {
	...
	it("should validate receipt for test Google product", function (done) {
		inappMock.configureNextGoogleRequest(false);
	...
	it("should not validate receipt for really bought Google product but inexisting in store config", function (done) {
	...
	it("should validate receipt for Google product", function (done) {
		request(shuttle)
			.post('/v1/gamer/store/validateReceipt')
			.set(dataset.validAppCredentials)
			.auth(gamer_id, gamer_token)
	...
	it("should retrieve the purchase history", function (done) {
		request(shuttle)
