...
describe('GameVFS', function() {
	...
	it("should write a counter named 'counter'", function(done){
		...
		.then(() => xtralife.api.gamevfs.readAsync(domain, "counter")).then(function(result){
			...
			return xtralife.api.gamevfs.incr({}, domain, "counter", 1);}).then(function(result) {
			result.counter.should.eql(1);
			return xtralife.api.gamevfs.readAsync(domain, "counter");}).then(result => result.counter.should.eql(1)).then(() => xtralife.api.gamevfs.delete(domain, "counter", function(err){
            if (err != null) { return done(err); }
            return done();
        })).catch(done);
		return null;
